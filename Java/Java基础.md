...
所以[并发编程](https://so.csdn.net/so/search?from=pc_blog_highlight&q=并发编程)的目标是充分的利用处理器的每一个核，以达到最高的处理性能。
...
start()方法来启动一个线程，真正实现了[多线程](https://so.csdn.net/so/search?from=pc_blog_highlight&q=多线程)运行。这时无需等待run方法体代码执行完毕，可以直接继续执行下面的代码； 这时此线程是处于就绪状态， 并没有运行。 然后通过此Thread类调用方法run()来完成其运行状态，  这里方法run()称为线程体，它包含了要执行的这个线程的内容， Run方法运行结束， 此线程终止。然后CPU再调度其它线程。
...
![img](https://img-blog.csdnimg.cn/20190317183139734.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3NoYWRvd196ZWQ=,size_16,color_FFFFFF,t_70)
...
![img](https://img-blog.csdnimg.cn/20190317183206697.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3NoYWRvd196ZWQ=,size_16,color_FFFFFF,t_70)
...
Java[反射机制](https://so.csdn.net/so/search?from=pc_blog_highlight&q=反射机制)主要提供了以下功能：
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20190403224221575.png)
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/QCu849YTaIP7hut9PibcGy8FUicnEa2BzcqCaW7HyibYgP9KAm1cQ5CnHHDD8Ey3iavuIviazJL8aYKlYMia5YIAnW2Q/640?tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIP7hut9PibcGy8FUicnEa2BzcWJ6E9QLpqz8RibgCuSYrJ087ibv3ALZvR98g8YRUkJ6TKJ40OLg61mSA/640?tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIP7hut9PibcGy8FUicnEa2BzcaM97J5ZLDA9yY4JwQEsBfjvFD9IxicMJdib0f18gcIslbsaSlQlEY5tA/640?tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIP7hut9PibcGy8FUicnEa2BzcWopOnCjT3BW2KsfQV9l8HC6Oia1XDpK5Zc3UWI8WqS2ibEtPYcNv0NQA/640?tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
hibernate工作原理：
 ![在这里插入图片描述](https://img-blog.csdnimg.cn/20190403231419411.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2xpMTMyNTE2OTAyMQ==,size_16,color_FFFFFF,t_70)**120. get()和 load()的区别？**
...
1. 说一下堆栈的区别？
2. 栈内存存储的是局部变量而堆内存存储的是实体；
3. 栈内存的更新速度要快于堆内存，因为局部变量的生命周期很短；
4. 栈内存存放的变量生命周期一旦结束就会被释放，而堆内存存放的实体会被垃圾回收机制不定时的回收。
5. 队列和栈是什么？有什么区别？
